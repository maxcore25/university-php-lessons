...
<body>
    ...
    <?php
    echo "<h1>This is an admin page of Library application</h1><br/><br/>";
    ...
    foreach ($books as $row) {
        ...
        <div class='card' style='width: 18rem;'>
          <div class='card-body'>
            ...
            <h6 class='card-subtitle mb-2 text-muted'>В наличии: {$row['amount']}</h6>
    ...
    $vacancy = $mysqli->query("SELECT * FROM vacancy");
    foreach ($vacancy as $row) {
        echo "
            <div class='card' style='width: 18rem;'>
            <div class='card-body'>
              <h5 class='card-title'>{$row["name"]}</h5>
              <h6 class='card-subtitle mb-2 text-muted'>Зарплата: {$row['salary']}</h6>
              <p class='card-text'>Lorem ipsum dolor sit amet.</p>
            </div>
          </div>
          ";
